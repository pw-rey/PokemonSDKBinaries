...
texture_patch="$root/patches/sfemovie-typed-texture.patch"
if ! patch -f --dry-run -R -d "$binding" -p1 < "$texture_patch" >/dev/null 2>&1; then
  patch -f -d "$binding" -p1 < "$texture_patch"
fi
